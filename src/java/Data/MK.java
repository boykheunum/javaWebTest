 ...
 * @author Admin
 */
@WebServlet(name = "MK", urlPatterns = {"/MK"})
public class MK extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(check==false){
                 out.print("<script>alert('Mat khau moi nhap ko giong nhau')</script>");
            }else{
                if(kq==-2){
                    out.print("<script>alert('Sua mat khau that bai')</script>");
                }else if(kq!=0){
                    out.print("<script>alert('Sua mat khau thanh cong')</script>");
                }
            }
        }
    }
